...
import pytest
from collection_argparse.cli import (NotFound, UnknownFunc, main, opening_file,
                                     parse)
...
@patch('collection_argparse.cli.opening_file')
@patch('collection_argparse.cli.list_input')
...
@patch('collection_argparse.cli.counter_unique_characters')
def test_args_string(mock_counter_func):
    assert main(argparse.Namespace(file=None, string='kokobdji'))
